...
export class EditProfileGroupModalComponent<G extends ProfileGroup> {
    ...
    /**
     * Maximum allowed folder nesting depth (number of levels).
     * When set, parent candidates are filtered so that neither this group nor
     * its own subtree can be pushed past that many levels.
     */
    @Input() maxDepth: number | undefined
    ...
    /**
     * Nesting depth of a group: 0 for a top-level folder
     */
    private depthOf (groups: PartialProfileGroup<ProfileGroup>[], id: string): number {
        let depth = 0
        let current = groups.find(g => g.id === id)
        let guard = 0
        while (current?.parentGroupId && guard++ < 30) {
            depth++
            const parentId: string = current.parentGroupId
    ...
    /**
     * Number of extra levels below a group (0 when it has no subfolders)
     */
    private subtreeHeight (groups: PartialProfileGroup<ProfileGroup>[], id: string, guard = 0): number {
        if (guard > 30) {
            return 0
        }
        ...
        return 1 + Math.max(...children.map(c => this.subtreeHeight(groups, c.id, guard + 1)))
    }
    ...
    async ngOnInit () {
        ...
        let candidates = this.getValidParents(groups, this.group.id)

        if (this.maxDepth) {
            // Moving this group under a candidate must not push this group
            // or any of its own subfolders past `maxDepth` levels.
            const movingHeight = this.subtreeHeight(groups, this.group.id)
            const maxParentDepth = this.maxDepth - 2 - movingHeight
            candidates = candidates.filter(g => this.depthOf(groups, g.id) <= maxParentDepth)
        }

        this.groups = candidates
