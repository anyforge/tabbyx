...
const TabbyX = {
    ...
    registerPluginModule: (packageName, module) => {
        TabbyX.registerModule(`resources/builtin-plugins/${packageName}`, module)
        TabbyX.registerModule(packageName, module)
    },
    loadPlugin: async (url) => {
        const info = await TabbyX.resolvePluginInfo(url)
        const module = await webRequire(info.url)
        TabbyX.registerPluginModule(info.name, module)
        return module
    },
    loadPlugins: async (urls, progressCallback) => {
        const infos: any[] = await Promise.all(urls.map(TabbyX.resolvePluginInfo))
    ...
    bootstrap: (...args) => window['bootstrapTabbyX'](...args),
    webRequire,
}
